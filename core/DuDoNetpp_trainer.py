...
from DuDoNetpp.DuDoNetppModel import DuDoNetpp
...
class DudoNetpp_Trainer(nn.Module):
    def __init__(self):
        super(DudoNetpp_Trainer, self).__init__()
    ...
    def initial(self, opt, device, train_dataset, train_loder, val_loder=None):
        ...
        self.MAR_Net = DuDoNetpp(opt).to(device)
        self.FP = ForwardProj
        self.device = device
        self.opt = opt
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            ...
            for Igt, Xma, _, Mask, Sgt, Sma,_ in self.val_loder:
                ...
                Sse, Ise, Iout = self.MAR_Net(Sma, Mp, Xma, Mask)
    ...
    def model_fitting(self):
        ...
        for epoch in range(self.opt.Epochs):
            ...
            trainloss = []
            # gt,metal,XLI,mask_onlymetal,Sgt,Sma, SLI
            for step,(Igt, Xma, _, Mask, Sgt, Sma,_) in loop:
                self.MAR_Net.train()
                Igt = Igt.to(device=self.device,dtype=dataType)
                Sgt = Sgt.to(device=self.device, dtype=dataType)
                Xma = Xma.to(device=self.device, dtype=dataType)
